<?php

//resources/lang/es/messages.php

return [
    'title' => 'Ideas en Acción',
    'sub' => 'TÚ INNOVACIÓN Y/O DESAFÍO <br> CUENTAN',
    'p1' => 'La convocatoria abierta del <strong> Premio Ideas en Acción </strong>
        invita a presentar  <strong>innovaciones implementadas</strong>  por empresas 
        prestadoras de servicios de agua y/o saneamiento en América Latina y el Caribe.
        <br><br>Este año, la convocatoria también invita a las empresas a presentar<strong> 
        desafíos </strong>que puedan ser atendidos a través de soluciones innovadoras en
        tecnologías inteligentes, desalinización, reúso y/o gestión.
        <br><br>En el 2020 el <strong>Premio Ideas en Acción </strong>consta de dos categorías.',
    'cats' => 'Categorías',
    'p2' => 'Para <strong>innovaciones
        implementadas</strong> por empresas prestadoras de servicios de agua y/o saneamiento en
        América Latina y el Caribe.',
    'p3' => 'Para empresas prestadoras
        de servicios de agua y/o saneamiento en América Latina y el Caribe con <strong>desafíos</strong>
        que puedan ser solucionados a través de la implementación de pilotos innovadores en tecnologías
        inteligentes, desalinización, reúso y/o gestión.',

    'part' => 'PARTICIPA',
    'dqst' => '¿De qué se trata?',
    'cat' => 'CATEGORÍA',
    'cat1' => 'Excelencia Innovadora Empresarial',

    'p4' => 'Busca iniciativas, prácticas y modelos innovadores creados y/o utilizados para 
        afrontar retos que tienen las empresas prestadoras de servicios de agua y/o 
        saneamiento en América Latina y el Caribe, que hayan sido probados e 
        implementados; y que hayan generado un impacto positivo en alguna de las 
        siguientes categorías:',

    'i1' => '<strong>Innovación <br>
        ambiental y social </strong>',
    'i2' => '<strong> Innovación <br>
        tecnológica</strong>',
    'i3' => '<strong> Innovación <br>
        en procesos </strong>',

    'p5' => '  Si tu empresa ha implementado un programa, práctica, producto, o servicio 
        innovador en el área ambiental, social, tecnológica o procedimental enfocado 
        en AGUA y/o SANEAMIENTO <strong> ¡esta categoría es para ti!</strong> ',
    'etp' => 'ENVÍA TU PROPUESTA',
    'byc' => 'VER BASES Y CONDICIONES DE ESTA CATEGORÍA',
    'pg' => 'Podrás ganar:',

    'p6' => 'Implementación de AquaRating +
        y actividades de formación, promoción,<br>
        visibilidad y difusión.',
    'cat2' => 'Desafíos',

    'p7' => 'Busca identificar desafíos en empresas operadoras de agua y/o
        saneamientos de América Latina y el Caribe que puedan ser atendidos
        mediante soluciones innovadoras en tecnologías inteligentes, desalinización, 
        reúso y/o gestión.',

    'i4' => '<strong>-	Tecnologías <br> Inteligentes <br> (Smart Water<br> Infrastructure<br> 
            Technologies -<br> (SWIT))</strong>',
    'i5' => '<strong> Desalinización </strong>',
    'i6' => '<strong> Tratamiento de <br>
        aguas residuales <br>
        y reúso.</strong>',
    'i7' => '<strong> Innovaciones <br>
        en gestión de los <br>
        servicios de agua y <br>
        aneamiento.</strong>',
        
    'p8' => 'Si tu empresa presenta un desafío en AGUA y/o SANEAMIENTO que puede
        ser atendido / solucionado con tecnologías innovadoras que correspondan a
        una o varias de las áreas de innovación descritas <strong> ¡esta categoría es para ti! </strong>',
    'p9' => 'Asistencia técnica para asesorías,
        estudios de prefactibilidad / factibilidad, 
        planes de negocios, y capacitación, 
        así como la implementación del proyecto piloto 
        enfocado a resolver el desafío formulado.',
    'contac' => 'Contáctenos',
    'p10' => '¿Tienes preguntas?<br>Si tienes alguna consulta,
        puedes contactarnos a <br> <a href="#" style="color: #04b3bb">
        agua@iadb.org</a> ó <a href="#" style="color: #04b3bb">argentina@socialab.com</a>',
    'name' => 'Nombre',
    'mail' => 'Dirección de correro electrónico',
    'msj' => 'Mensaje',
    'send' => 'ENVIAR',
    'img1' => 'images/img--02.jpg',
    'imgcat1' => 'images/img--09.jpg',
    'imgcat2' => 'images/img--12.jpg',
    'bandera' => 'flag-icon-es',
    'banderaoff' => 'flag-icon-gb',
    'linkbanderaoff'=>'/lang/en',
    'onClick'=>'cambiarImgEn()',

    //PREGUNTAS FRECUENTES / FAQ
    'pf'=>'Preguntas Frecuentes',
    //PREGUNTAS GENERALES
    'prge'=>'Preguntas<br>
             generales',
    'pr1'=>'En el 2020, Ideas en Acción cuenta con dos categorías, ¿Puede mi empresa participar en las dos categorías?',
    'pr2'=>'¿En qué idioma puedo enviar mi(s) postulación(es)?',

    //Categoría: Excelencia Innovadora Empresarial
    'cie'=>'Categoría Excelencia<br> Innovadora Empresarial',
    'pr3'=>'    La iniciativa de la empresa todavía no está implementándose o ejecutándose, pero el plan de negocios y 
           los socios ya están listos. ¿Puedo participar?',
    'pr4'=>'  Son varias preguntas las que hay que responder en el formulario. ¿Debo responderlas todas?',
    'pr5'=>' ¿Necesito la carta de autorización de la empresa o puedo tener solamente un email de aprobación?',
    'pr6'=>' ¿El video será publicado en los canales de comunicación del BID?',
    'pr7'=>' ¿El video será usado únicamente dentro del tiempo y marco de esta convocatoria?',
    'pr8'=>' ¿Otras empresas podrán ver mi video? ',
    'pr9'=>' ¿Debo incluir en la propuesta y video cómo la solución innovadora aporta a la
        minimización del impacto del Coronavirus (COVID-19) en las operaciones de la empresa?',
    'pr10'=>'¿Debo incluir en la propuesta y video cómo la solución innovadora ha implementado exitosamente un sistema de 
        infraestructura verde para el control de inundaciones en contexto urbano?',
    
    //Categoría: Desafío
    'cc'=>'Categoría Desafío',
    'pr11'=>'Mi desafío puede ser atendido a través de una o varias soluciones 
        innovadoras en las áreas descritas ¿Puedo seleccionar una o varias áreas en el formulario de aplicación?',
    'pr12'=>'Mi desafío puede ser atendido a través de otra área de tecnológica ¿Puedo participar en esta categoría?',
    'pr13'=>'¿Necesito la carta de presentación del desafío de la empresa?',

    //RESPUESTAS
        //PREGUNTAS GENERALES
    'pg'=>'Preguntas generales',
    'r1'=>'Si. Las empresas prestadoras de servicios de agua y/o saneamiento pueden aplicar a una o a ambas 
        categorías del Premio Ideas en Acción 2020. Asegúrate de revisar las bases y condiciones de cada una de las
         categorías y enviar tu propuesta de innovación y/o desafío en la categoría respectiva.<br><br>

         <strong>CATEGORÍA: EXCELENCIA INNOVADORA EMPRESARIAL</strong>.Para<strong>innovaciones implementadas</strong>por empresas
          prestadoras de servicios de agua y/o saneamiento en América Latina y el Caribe. Hasta un máximo 
          de tres (3) innovaciones por empresa. <br><br>

          <strong> CATEGORÍA: DESAFÍOS</strong>. Para empresas prestadoras de servicios de agua y/o saneamiento en América Latina 
          y el Caribe con <strong>desafíos</strong> que puedan ser solucionados a través de la implementación de pilotos innovadores
           en tecnologías inteligentes, desalinización, reúso y/o gestión. Hasta un máximo de dos (2) desafíos
            por empresa. ',
    'r2'=>'La(s) postulación(es) a las dos categorías podrán ser enviadas en cualquiera de los 
            cuatro idiomas oficiales del BID: español, francés, inglés o portugués.',
    
      //Categoría: Excelencia Innovadora Empresarial        
    'r3'=>'En esta convocatoria se evaluarán y seleccionarán únicamente iniciativas innovadoras que han sido
             implementadas en los últimos 5 años y que pueden demostrar resultados. ',
    'r4'=>'Sí, todas las preguntas obligatorias deben ser contestadas respetando el límite de palabras en cada campo. 
            Es importante mencionar resultados concretos, cifras, indicadores, e impacto.',
    'r5'=>'Sí, es necesario presentar la carta de autorización en las condiciones descritas en los términos y condiciones.',
    'r6'=>'Los videos de participación podrán ser usados en diferentes canales de comunicación o escenarios del BID
            en el marco de la convocatoria o en general en el marco de gestión de innovación del BID. 
            Al enviar la solicitud los participantes consienten en ceder al BID el derecho de uso de dichos
            videos para propósitos promocionales y de difusión de los premios.',
    'r7'=>'El material podrá ser usado para diferentes propósitos dentro y fuera
             del marco de esta convocatoria con fines instructivos, educativos, etc.',
    'r8'=>'Sí, los videos deberán ser publicados en canales de comunicación con acceso abierto.',
    'r9'=>'Esta pregunta es opcional. Si la iniciativa innovadora aborda soluciones orientadas a minimizar el impacto
        del coronavirus (COVID-19) en la continuidad de las operaciones de las empresas de agua y/o saneamiento de manera
        original, déjanos saber en la postulación y video como la iniciativa, práctica o modelo contribuye a responder a
        esta problemática. ',
    'r10'=>'Esta pregunta es opcional. Si la iniciativa, práctica o modelo ha implementado exitosamente un sistema 
            de infraestructura verde para el control de inundaciones en un contexto urbano, déjanos saber en la postulación 
            y video como lo ha hecho y cuales han sido los resultados.',

    //Categoría: Desafío
    'r11'=>'Si. Se pueden seleccionar mínimo una (1) o hasta un máximo de cuatro (4) áreas. Se busca identificar desafíos 
        que puedan ser atendidos mediante innovaciones en una, o una combinación de las siguientes áreas tecnológicas
        y/o de gestión:<br><br>

        <ul>
            <li>Tecnologías Inteligentes (Smart Water Infrastructure Technologies (SWIT))</li>
            <li>Desalinización</li>
            <li>Tratamiento de aguas residuales y reúso</li>
            <li>Innovaciones en gestión de los servicios de agua y saneamiento.</li>
        </ul>',
    'r12'=>'En esta convocatoria, Categoría Desafíos, se evaluarán y seleccionarán únicamente desafíos que puedan 
            ser solucionados a través de la implementación de pilotos innovadores en tecnologías inteligentes, 
            desalinización, reúso y/o gestión.',
    'r13'=>'Sí, es necesario enviar a través de la plataforma la carta de presentación del desafío en las condiciones
             descritas en los términos y condiciones. ',
];